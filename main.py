...
# parser.add_argument('--data_path', default='/esat/pyrite/wvangans/Datasets/KITTI/Depth_Completion/data/', help='path to desired dataset')
...
def main():
    ...
    save_id = '{}_{}_{}_{}_{}_batch{}_pretrain{}_wlid{}_wrgb{}_wguide{}_wpred{}'.\
    ...
    if not args.no_tb:
        # Tensorboard writer
        global writer
        writer = SummaryWriter(os.path.join('Tensorboard', save_id))
        # dummy_input = torch.rand((args.batch_size, channels_in, args.crop_h, args.crop_w), requires_grad=True).float().cuda()
    ...
    criterion_local = define_loss(args.loss_criterion, args.mask_intersec)
    criterion_lidar = define_loss(args.loss_criterion, args.mask_intersec)
    criterion_rgb = define_loss(args.loss_criterion, args.mask_intersec)
    criterion_guide = define_loss(args.loss_criterion, args.mask_intersec)
    ...
    # Define variables
    weight_map = None
    factor = args.max_depth // args.num_bins
    ...
    for epoch in range(args.start_epoch, args.nepochs):
        ...
        for i, (input, gt) in tqdm(enumerate(train_loader)):
            ...
            weight_map = torch.clamp(torch.ceil(gt/factor), min=0, max=args.num_bins)
            # if args.normal:
                # prediction = prediction*args.max_depth
