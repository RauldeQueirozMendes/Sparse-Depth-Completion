...
def main():
    ...
    with torch.no_grad():
        ...
        #                                    dataset.selected_paths['img'], dataset.selected_paths['gt']))):
            ...
            net_input = torch.unsqueeze(raw, 0).cuda()
            gt = torch.unsqueeze(gt, 0).cuda()

            if args.normal:
                # Put in {0-1} range and then normalize
                net_input = net_input/args.max_depth
                # net_input = depth_norm(net_input)
            ...
            if args.input_type == 'rgb':
                ...
                net_input = torch.cat((net_input, rgb), 1)

            torch.cuda.synchronize()
            a = time.perf_counter()
            output, _, _, _ = model(net_input)
            torch.cuda.synchronize()
            b = time.perf_counter()
